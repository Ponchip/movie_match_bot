...
async def get_db() -> aiosqlite.Connection:
    ...
    if _db_connection is None or _db_connection.closed:
        ...
        # Включаем поддержку внешних ключей и оптимизацию
...
async def init_db():
    ...
    await db.execute("CREATE INDEX IF NOT EXISTS idx_swipes_user ON swipes(user_id)")
    await db.execute("CREATE INDEX IF NOT EXISTS idx_shown_user ON shown_movies(user_id)")
    await db.commit()
...
async def save_movie(movie_data: dict) -> int:
    db = await get_db()
    await db.execute("""
        INSERT OR IGNORE INTO movies (tmdb_id, title, year, rating, poster_url, description) 
        VALUES (?, ?, ?, ?, ?, ?)
    """, (
